	...
	MobileCalendar.prototype = {
		init: function(params) {
			...
			if (this.auto) {
				...
				this.target = params.trigger;
				var input = document.createElement("input");
				input.type="text";
				input.style.cssText = "display:none";
				// debugger;
				input.setAttribute("value", params.trigger.value ) ;

				this.target.parentNode.appendChild( input );
				this.trigger = input;
				// this.trigger = $('<input type="text" style= value="'+params.trigger.innerHTML+'" />')[0];
			...
			this.bindEvent(this.type, this.auto);
		},
		bindEvent: function(type, auto) {
			var _self = this;
			// chen
			var autoModel = "";
			if (auto) {
			...
			function popupDate(e) {
				...
				_self.gearDate.className = "gearDate "+autoModel;
	 			_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
					...
					'</div>' +
					'</div>';
				_self.wrapper.appendChild(_self.gearDate);
				dateCtrlInit();
			...
			function setGear(target, val) {
				...
				if (auto) {
					({
						"ym": finishMobileYM,
						"date": finishMobileDate,
						"datetime": finishMobileDateTime,
						"time": finishMobileTime
					}[type]());
			...
			function finishMobileDate(e) {
				...
				// 选择性改变显示时间
				if ( auto ) {
				   _self.target.value =  _self.trigger.value ;
				}else{
					closeMobileCalendar(e);
				}
				// 触发回调
				_self.onChange && _self.onChange( _self.trigger.value );
			...
			_self.trigger.addEventListener('click', runPicker);
			...
			function clearPicker(){
				if (_self.wrapper.querySelector(".gearDate")) {
					var evt;
					try {
					...
					_self.wrapper.removeChild(_self.gearDate);
					_self.gearDate=null;
				}
			}
			...
			function refreshTo(params){
				...
				if ( typeof params === "string" ) {
					...
					if (params.date) {
						_self.trigger.value = params.date;
					}
				}
		...
		// 销毁实例
		destory: function () {
			if (this.wrapper.querySelector(".gearDate")) {
				var evt;
				try {
					evt = new CustomEvent('input');
				} catch (e) {
				...
				this.wrapper.removeChild(this.gearDate);
				this.gearDate=null;
			}
			if (this.auto) {
